...
The error term vector $\epsilon_t$ given the data up to $t-1$ is assumed to be multivariate normally distributed of dimension $N=10$, with mean $0_N$ and covariance matrix $\Sigma$.

Bayesian estimation techniques will be then utilised in conjunction with suitably chosen priors in order to compute 1 day ahead RV forecasts across all indices. The accuracy of the forecasts will then be assessed according to their root mean squared errors **RMSE** given by:
...
Comparison of prediction accuracy via the RMSEs will facilitate comparison of various different model specifications, such as the incorporation of different prior distributions and differing assumptions about the parameters of those distributions.
