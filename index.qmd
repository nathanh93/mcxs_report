...
res = get_posteriors(300, Y_ext, X_ext, pris)
...
preds_ext = get_predictions(300, 20, A.posterior.draws, Sigma.posterior.draws)
preds_base = get_predictions(300, 20, res[[1]], res[[2]])

```

The respective out of sample forecasts for 20 days ahead are as follows. The forecasts from the baseline model are plotted in blue and the T-distributed error model forecasts are plotted in red. 90% confidence intervals are denoted by the shaded regions.
...
We can transform the variables and forecast back into daily RVs by taking the exponent of the variables. The RV forecasts can then be visualised as follows:
